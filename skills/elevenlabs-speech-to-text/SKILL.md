...
Save transcript text, structured JSON and any requested timestamps/subtitles locally. For a returned `transcription_id`, use [`DELETE /v1/speech-to-text/transcripts/{transcription_id}`](https://elevenlabs.io/docs/api-reference/speech-to-text/delete) and verify the exact resource is unavailable via the corresponding GET. For realtime sessions without a deletable transcript ID, consult the current endpoint's retention support; do not invent a history ID or claim that closing the socket deletes server data.
