...
description: Transform an existing recording into another existing ElevenLabs voice while preserving timing and delivery. Use for voice conversion, re-voicing narration, and speech-to-speech audio. Does not create or clone a voice.
license: MIT
...
For this skill, capture `history-item-id` when returned. Otherwise use the response `request-id` to match the exact item in [`GET /v1/history`](https://elevenlabs.io/docs/api-reference/history/list), following pagination. Do not substitute a request ID for a history item ID or guess from matching text. Use [`DELETE /v1/history/{history_item_id}`](https://elevenlabs.io/docs/api-reference/history/delete), then verify the exact item is no longer available through `GET /v1/history/{history_item_id}`. If attribution is ambiguous, report the unresolved cleanup instead of deleting a candidate.
...
If the input recording is noisy, either pre-process with the elevenlabs-voice-isolator skill or pass `remove_background_noise=True` to do it in a single call:
...
- **Clean up noise first.** Either pass `remove_background_noise=True` or run the source through the elevenlabs-voice-isolator skill before conversion. Noise hurts more here than in TTS.
