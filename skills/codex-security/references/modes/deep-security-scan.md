...
2. run repeated independent discovery workers, each of which generates its own repository-level threat model before `references/phases/finding-discovery.md`
...
5. run `references/phases/validation.md`, `references/phases/attack-path-analysis.md`, and final report assembly once

Do not replace Codex Security's established scan rules with custom shortcuts.

## Required Capabilities

Before starting, confirm that the Codex Security local mode and phase references needed by this workflow are available:

- `references/modes/security-scan.md`
- `references/phases/threat-model.md`
- `references/phases/finding-discovery.md`
- `references/phases/validation.md`
- `references/phases/attack-path-analysis.md`

If any required reference is unavailable, stop and say that this Codex Security installation does not include the required scan workflow. Do not silently degrade into a different workflow.
...
1. Read `references/modes/security-scan.md` first and follow its repository-wide scan semantics exactly.
2. Resolve the full-repository target once. Deep Security Scan v0 does not support PR diffs, commits, branch diffs, working-tree diffs, or scoped paths. If the user requested a scoped path, stop this workflow and direct them to ordinary `references/modes/security-scan.md`; if the user requested a PR, commit, branch diff, or working-tree diff, direct them to ordinary `references/modes/security-diff-scan.md`. Do not silently widen the scope.
...
- no top-level `references/phases/validation.md`, no top-level `references/phases/attack-path-analysis.md`, and no final report assembly
- no file edits
...
First generate your own repository-level threat model for that resolved target using the ordinary `references/phases/threat-model.md` rules, but write it only to your worker-specific threat-model output path. Do not read, reuse, overwrite, or infer a shared coordinator threat model.
Then run `references/phases/finding-discovery.md` using your own worker-specific repository threat model as the threat-model source of truth.
Do not reinterpret the target, run the top-level `references/phases/validation.md` phase, run the top-level `references/phases/attack-path-analysis.md` phase, assemble the final report, or edit repository files.
...
Apply the ordinary `references/phases/finding-discovery.md` rules in full:
...
This report is not a selective promotion list, triage summary, or second consolidation layer. It is the lossless canonical merged candidate set in the same artifact shape that ordinary `references/phases/finding-discovery.md` would hand to validation.
...
Write the canonical consolidated versions back to the numbered standard paths above so `references/phases/validation.md` receives the normal repository-wide inputs it expects.
...
3. confirm `<context_dir>/threat_model.md` exists, then run `references/phases/validation.md` once over the canonical merged discovery inputs
4. run `references/phases/attack-path-analysis.md` once over the surviving validated findings and closure rows that require it
