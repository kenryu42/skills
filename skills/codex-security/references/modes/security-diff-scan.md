...
4. Run `references/phases/finding-discovery.md` as the second step, against the resolved diff and using the per-scan threat model as context.
...
5. Run `references/phases/validation.md` as the third step, for each candidate that came out of discovery.
...
6. Run `references/phases/attack-path-analysis.md` as the fourth step, for findings that still need reportability, attack-path, and severity analysis after validation.
...
- copy every diff row into `deep_review_input.csv` with `python3 <plugin_dir>/scripts/generate_rank_input.py copy-deep-review-input --rank-input <discovery_dir>/rank_input.csv --out <discovery_dir>/deep_review_input.csv`
