...
Treat this mode as the top-level orchestrator for the four phase references plus the final report assembly step. Do not collapse the phases together.

For each phase:
1. Read that phase's reference.
...
3. Run `references/phases/threat-model.md` first.
...
4. Run `references/phases/finding-discovery.md` as the second step, against the resolved repository or scoped path and using the per-scan threat model as context.
...
5. Run `references/phases/validation.md` as the third step, for each candidate that came out of discovery and each open, reportable, or deferred seeded/root-control ledger row that still needs closure.
...
6. Run `references/phases/attack-path-analysis.md` as the fourth step, for findings and validation closure rows that still need reportability, attack-path, and severity analysis after validation.
...
- Before the `references/phases/finding-discovery.md` phase, read `references/repository-wide-scan.md` and every required reference it lists, then use them for finding discovery, validation, and attack path analysis.
